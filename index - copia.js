...
dbUsuarios.get(consulta, [datoRecibido, estatusDelPedido], (err, fila) => {
  ...
  if (fila) {
    ...
    res.json({ mensaje: statusEnvio, estado: estadoPrep, hora:horaEstimada });
...
// Ruta para obtener mensajes mediante SSE
app.get("/mensajes-sse", (req, resp) => {
  resp.setHeader("Content-Type", "text/event-stream");
  resp.setHeader("Cache-Control", "no-cache");
  resp.setHeader("Connection", "keep-alive");

  // Función para enviar datos al cliente a través de SSE
  const enviarDatosSSE = (datos) => {
      resp.write(`data: ${JSON.stringify(datos)}\n\n`);
  };
  ...
  const intervalId = setInterval(() => {
      ...
      db.all(query, function(err, rows) {
          if (err) {
              ...
              // Enviar los datos al cliente a través de SSE
             // console.log(rows);
              enviarDatosSSE(rows);
          }
      });
  ...
  req.on("close", () => {
      clearInterval(intervalId); // Detener el intervalo cuando el cliente se desconecta
      resp.end();
  });
});
